...
def increase_block_height():
    ...
    while True:
        ...
        filename = 'status.json' if os.path.exists('status.json') else 'status_renamed.json'
        with open(filename) as json_read_file:
        ...
        with open(filename, 'w') as json_write_file:
